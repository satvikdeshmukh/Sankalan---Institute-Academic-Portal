...
import { ArrowLeft, FileText, Download, FolderOpen, Loader2 } from 'lucide-react';
...
export default function PrincipalHODDetail() {
    ...
    useEffect(() => {
        ...
        apiFetch(`/principal/user/${hodId}`)
            ...
            .catch(err => {
                console.error('HODDetail load error:', err);
                setError('Failed to load HOD profile');
            })
    ...
    const initials = name.split(' ').filter(Boolean).map(n => n[0]).join('').toUpperCase().slice(0, 2) || 'H';

    return (
        <DashboardLayout title="About HOD" subtitle="HOD profile and uploaded documents">
            <button
                onClick={() => navigate('/principal/hods')}
                className="flex items-center gap-2 text-sm text-gray-500 hover:text-violet-600 font-medium mb-6 transition-colors"
            >
                <ArrowLeft className="w-4 h-4" /> Back to HODs
            </button>
                ...
                <div className="flex flex-col lg:flex-row gap-6 items-start">
                    ...
                    <div className="w-full lg:w-56 flex-shrink-0">
                        <div className="bg-white rounded-2xl shadow-card p-6 flex flex-col items-center text-center">
                            <div className="w-24 h-24 rounded-2xl overflow-hidden shadow-lg mb-4 ring-4 ring-violet-50">
                            ...
                            <div className="mt-3 text-xs text-gray-500 space-y-0.5 text-left w-full border-t border-gray-100 pt-3">
                                <p><span className="font-medium text-gray-600">Role:</span> HOD</p>
                                {profile?.email && (
                    ...
                    <div className="flex-1 space-y-6">
                        <div className="bg-white rounded-2xl shadow-card p-6">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input readOnly value={profile?.yearsExperience ?? profile?.experience_years ?? ''} placeholder="—" className={inputCls} />
                                </div>
                        ...
                        <div className="bg-white rounded-2xl shadow-card overflow-hidden">
                            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                                ...
                                <span className="text-xs bg-violet-50 text-violet-700 font-semibold px-2.5 py-1 rounded-lg">
