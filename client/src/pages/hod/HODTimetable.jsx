...
function DeptDetailView({ dept, summaries, teachers, departments, onBack, showToast, selectedYearSec }) {
    ...
    // Fetch department teachers via authorized HOD route
    useEffect(() => {
        api.get('/hod/teachers').then(data => setAllUsers(data || [])).catch(() => {});
    }, []);
    ...
    const filteredTeachers = useMemo(() => {
        return allUsers.filter(t => {
            // Support both Admin (id) and HOD (user_id) data formats
            const tid = t.user_id || t.id;
            if (!assignedTeacherIds.has(tid)) return false;
            
            const name = (t.full_name || t.fullName || '').toLowerCase();
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center gap-4 flex-wrap">
                ...
                <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl bg-[linear-gradient(135deg,_rgb(180,83,9)_0%,_rgb(217,119,6)_100%)] flex items-center justify-center"><Building2 className="w-5 h-5 text-white" /></div>
                    <div>
                ...
                {[{ id: 'class', label: '🏫 Class-wise' }, { id: 'teacher', label: '👨‍🏫 Teacher-wise' }].map(tab => (
                    <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                        className={`px-5 py-2 rounded-xl text-sm font-semibold transition-all
                            ${activeTab === tab.id ? 'bg-[linear-gradient(135deg,_rgb(180,83,9)_0%,_rgb(217,119,6)_100%)] text-white shadow-md' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                        {tab.label}
                    </button>
                ))}
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-4">
                    ...
                    <div className="flex items-center gap-3 " >
                        <div className="w-9 h-9 rounded-xl bg-indigo-600 flex items-center justify-center" style={{ background: HERO_GRAD }}><Building2 className="w-5 h-5 text-white"  /></div>
                        <div>
                            <h2 className="font-bold text-gray-900 text-lg">{dept.name}</h2>
                ...
                <button onClick={() => setShowAddModal(true)}
                    className="flex items-center gap-2 px-4 py-2 bg-indigo-600 text-white rounded-xl text-sm font-semibold hover:bg-indigo-700 shadow-sm transition-all" style={{ background: HERO_GRAD }}>
...
export default function AdminTimetable() {
    ...
    const loadData = useCallback(async () => {
        ...
        try {
            ...
            const [depts, summs, principalData, user] = await Promise.all([
                api.get('/auth/departments'),
                api.get('/timetable/summary'),
                api.get('/principal/stats').catch(() => ({ teachers: [] })),
                api.get('/auth/me') // to know HOD department
            ]);
            ...
            // Find the HOD department
            const hodDept = (depts || []).find(d => d.id === user?.departmentId);

            setSelectedDept(hodDept);
    ...
    return (
        ...
        <DashboardLayout
            ...
            subtitle="Create, edit and publish department timetables"
        >
